...
async def test_chest_game_win_and_cashout(dut):
    ...
    doel_kist = 0
    alle_kisten = int(dut.chest_contents.value)
    for i in range(3):
        # Verschuif de bits (0, 3, of 6 plekken) en pak de onderste 3 bits (& 7)
        inhoud = (alle_kisten >> (i * 3)) & 7
        if inhoud == O_COIN:
            doel_kist = i
            break
...
async def test_chest_game_CURSED(dut):
    ...
    huidige_pot = int(dut.pot.value)
    huidige_ronde = int(dut.round.value)
    assert huidige_pot == 40, f"Pot zou 40 moeten zijn, maar is {huidige_pot}"  # interne pot variabele wordt niet verhoogd met 60, enkel pot_payout
    assert huidige_ronde == 1, f"Ronde zou 1 moeten zijn, maar is {huidige_ronde}"
